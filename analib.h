...
void DoHists(string DataName = ""){
	...
	TH2D * AQ = new TH2D("AQ","Voltage/sum",bins, 0, Qtop, bins,0,max_ampl);
	...
	QA_ratio = QA_ratio / (nentries - throwed);
	cout<<"From "<<nentries<<" wfm's, "<<throwed<<" was deleted"<<endl;
	cout<<"Q/A ratio = "<<QA_ratio<<endl;
	//~ TObject oQA_ratio;
	//~ oQA_ratio.SetUniqueID(QA_ratio);
	//~ oQA_ratio.Write("Q/A_ratio");
	TParameter<double> * pQA_ratio = new TParameter("QA_ratio",QA_ratio);
...
void PlotHists(string DataName = ""){
	...
	c1->cd(2);
	TH2D * AQ = (TH2D*) f->Get("AQ");
	AQ->Draw("colz");
	
	double QA_ratio = ((TParameter<double> *) f->Get("QA_ratio"))->GetVal();
	TH1D * h_Ampl_scaled = (TH1D *) h_Ampl->Clone("h_Ampl_scaled");
	h_Ampl_scaled->SetBins( h_Ampl_scaled->GetNbinsX(), 0, QA_ratio * h_Ampl_scaled->GetXaxis()->GetXmax());
	...
	for (int i = 0; i < h_QxA->GetNbinsX(); ++i) h_QxA->SetBinContent(i, h_QxA->GetBinContent(i) * h_Ampl_scaled->GetBinContent(i));
...
int GetQ1(TH1D * h_Q, TH1D * h_A, double QA_ratio){
	TH1D * h_QxA = (TH1D *) h_A->Clone("h_Ampl_scaled");
	for (int i = 0; i < h_QxA->GetNbinsX(); ++i) h_QxA->SetBinContent(i, h_QxA->GetBinContent(i) * h_A->GetBinContent(i));
	return h_QxA->GetMaximumBin();
}
...
void DoFit(string DataName = ""){
	...
	TF1 *fitfun = new TF1("fitfun",Fun, Q0,Q->GetXaxis()->GetXmax(),6);
	
	fitfun->SetLineColor(1);
	fitfun->SetLineWidth(2);
	
	fitfun->SetParNames("Q1","sig1","w","alpha","mu","A","Q0","sig0");
	...
	fitfun->SetParameter("Q1", GetQ1(Q, h_A, ((TParameter<double> *) f->Get("QA_ratio"))->GetVal()));
...
void FitHist( TH1D * h , json cfg_fit){
	...
	TF1 *fitfun = new TF1("fitfun",Fun, Q0, h->GetXaxis()->GetXmax(),6);
